		...
		switch ($packet->pid()) {
				...
				switch ($packet->status) {
					case 0:
						if ($player->getGamemode() === 1) {
							...
							$pk = new PlayerActionPacket();
							$pk->entityRuntimeId = $player->getId();
							$pk->action = PlayerActionPacket::ACTION_START_	break;
							$pk->x = $packet->x;
							$pk->y = $packet->y;
							$pk->z = $packet->z;
							$pk->face = $packet->face;
							...
							if ($block->getHardness() === (float) 0) {
								$pk = new PlayerActionPacket();
								$pk->entityRuntimeId = $player->getId();
								$pk->action = PlayerActionPacket::ACTION_STOP_	break;
								$pk->x = $packet->x;
								$pk->y = $packet->y;
								$pk->z = $packet->z;
								$pk->face = $packet->face;
								...
								$pk = new PlayerActionPacket();
								$pk->entityRuntimeId = $player->getId();
								$pk->action = PlayerActionPacket::ACTION_ABORT_	break;
								$pk->x = $packet->x;
								$pk->y = $packet->y;
								$pk->z = $packet->z;
								$pk->face = $packet->face;
			...
			case InboundPacket::ANIMATE_PACKET:
				...
				if (!$pos[0]->equals(new Vector3(0, 0, 0))) {
					...
					$pk = new PlayerActionPacket();
					$pk->entityRuntimeId = $player->getId();
					$pk->action = PlayerActionPacket::ACTION_CONTINUE_	break;
					$pk->x = $pos[0]->x;
					$pk->y = $pos[0]->y;
					$pk->z = $pos[0]->z;
					$pk->face = $pos[1];
